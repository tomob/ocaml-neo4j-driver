(* Unit tests for Cluster (minimal routing) with mock servers: address
   selection per access mode and the router fallback when a router's ROUTE
   fails. *)
...
(* Each role round-robins independently: with two readers and one writer, a
   read/write/read/write sequence alternates the readers and keeps hitting the
   single writer (a shared counter would always land on reader 0). *)
let per_role_round_robin () =
  ...
  Test_mock.with_servers 4
    ...
    (fun net clock sw ports ->
      ...
      check (list int) "read/write round robin" [ p1; p3; p2; p3 ]
...
let tests =
  [
    ...
    ( "[Cluster] per-role round robin",
      [ test_case "independent role counters" `Quick per_role_round_robin ] );

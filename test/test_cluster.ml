...
let per_role_least_loaded () =
  ...
  Test_mock.with_servers 4
    ...
    (fun net clock sw ports ->
      ...
      in
      let acquire mode =
        Cluster.acquire cluster ~mode ~database:None ~imp_user:None ~bookmarks:[] ~session_auth:None
  ...
  Test_mock.with_servers 3
    (fun ports ->
      ...
      [
        ...
        (* reader/router C: the first pool connection, the fallback ROUTE (whose
           connection is then handed to C's data pool and reused by the second
           acquire, since C doubles as a reader) *)
        [
          ((5, 0), List.nth received 2, [ Test_mock.Success ]);
          ( (5, 0),
            List.nth received 2,
      ...
      [
        ...
        [
          ( (5, 0),
            ...
            [
              Test_mock.Success;
              Test_mock.Success_meta [ ("rt", rt [ a ] [ "127.0.0.1:1" ] [ c ]) ];
              Test_mock.Success_meta [ ("rt", rt [ a ] [ "127.0.0.1:1" ] [ d ]) ];
            ] );
        ];
      ...
      let c1 =
        match
          Cluster.acquire cluster ~mode:Config.Read ~database:None ~imp_user:(Some "a")
            ~bookmarks:[] ~session_auth:None ~force_liveness:false
        with
        | Ok (conn, effective) ->

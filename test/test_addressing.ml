...
let parse_ok s expected =
  ...
  | Ok address ->
      check string ("parse " ^ s) expected (Addressing.to_string address)
...
let parse () =
  ...
  | Ok address ->
      check string "custom port default" "x:1234" (Addressing.to_string address)
  | Error error -> fail (Errors.to_string error));
  (match Addressing.parse ~default_host:"db" ~default_port:1234 "" with
  | Ok address ->
      check string "custom host default" "db:1234"
        (Addressing.to_string address)
  | Error error -> fail (Errors.to_string error));
...
let uri_ok uri expected_scheme expected_host expected_port =
  ...
  | Ok parsed ->
      check string "scheme" expected_scheme
        (Addressing.scheme_to_string parsed.scheme);
...
let routing_context () =
  ...
  | Ok context ->
      check
        (list (pair string string))
        "decoded"
        [ ("name", "neo4j driver"); ("k", "a b") ]
        context
  | Error error -> fail (Errors.to_string error)

let resolved () =
  let resolved =
    Addressing.make_resolved (Addressing.IPv4 ("10.0.0.1", 7687)) "example.com"
  in
  check string "resolved host name" "example.com"
    (Addressing.resolved_host_name resolved);
  check string "unresolved" "example.com:7687"
    (Addressing.to_string (Addressing.unresolved resolved));
  let resolved6 =
    Addressing.make_resolved
      (Addressing.IPv6 ("::1", 7687, 0, 0))
      "db.example.com"
  in
...
let tests =
  [
    ...
    ( "[Adressing] routing_context",
      [ test_case "routing context" `Quick routing_context ] );

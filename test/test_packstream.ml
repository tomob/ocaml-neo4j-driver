...
let byte_vector () =
  ...
  check string "empty string" "\x80" (Bytes.to_string (Packstream.pack (String "")));
...
let unknown_marker () =
  ...
  | Error e ->
      check string "unknown marker" "Unknown PackStream marker 0xC4" (Packstream.error_to_string e)

let map_key_not_string () =
  match Packstream.unpack (Bytes.of_string "\xa1\x01\xc0") with
  | Ok _ -> fail "non-string map key should fail"
  | Error e -> check string "map key" "Map key must be a string" (Packstream.error_to_string e)

let depth_limit () =
  let nested = List [ List [ List [ Null ] ] ] in
  (match Packstream.unpack ~limits:{ max_depth = 2 } (Packstream.pack nested) with
  ...
  match Packstream.unpack ~limits:{ max_depth = 3 } (Packstream.pack nested) with
  | Ok _ -> ()
  | Error e -> fail (Packstream.error_to_string e)
...
let bogus_length () =
  ...
  | Error e ->
      check string "oversized string" "Unexpected end of data" (Packstream.error_to_string e));
  ...
  | Error e -> check string "oversized list" "Unexpected end of data" (Packstream.error_to_string e)

let tests =
  [
    ("[Packstream] round_trip", [ test_case "all value types" `Quick round_trip ]);
    ("[Packstream] byte_vector", [ test_case "known encodings" `Quick byte_vector ]);
    ("[Packstream] truncated", [ test_case "truncated data" `Quick truncated ]);
    ("[Packstream] unknown_marker", [ test_case "unknown marker" `Quick unknown_marker ]);
    ("[Packstream] map_key_not_string", [ test_case "map key" `Quick map_key_not_string ]);
    ("[Packstream] depth_limit", [ test_case "depth limit" `Quick depth_limit ]);
    ("[Packstream] bogus_length", [ test_case "bogus lengths" `Quick bogus_length ]);
  ]

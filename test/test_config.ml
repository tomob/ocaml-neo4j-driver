...
let defaults () =
  ...
  check (float 1e-9) "retry delay jitter factor" 0.2 workspace.retry_delay_jitter_factor;
  check int "fetch size" 1000 workspace.fetch_size;
  check (option string) "database default" None workspace.database;
  check (option string) "impersonated user default" None workspace.impersonated_user;
  check bool "auto commit retries enabled" false workspace.disable_auto_commit_retries;
...
let tests =
  [
    ...
    ("[Config] access_mode", [ test_case "default access mode" `Quick access_mode ]);

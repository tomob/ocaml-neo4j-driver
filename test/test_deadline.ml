...
let infinite () =
  ...
  check (option (float 1e-9)) "no original timeout" None (Deadline.original_timeout d);
...
let finite () =
  ...
  check (option (float 1e-9)) "original timeout" (Some 10.0) (Deadline.original_timeout d);
...
let expired () =
  ...
  check (option (float 1e-9)) "clamped to zero" (Some 0.0) (Deadline.to_timeout d)
...
let merge () =
  ...
  | Some m -> check (option (float 1e-9)) "earliest" (Some 5.0) (Deadline.original_timeout m)
  ...
  | None -> ());
  (match Deadline.merge [] with Some _ -> fail "merge of empty list should be None" | None -> ());
  match Deadline.merge_and_timeouts [ None; Some 3.0 ] with
  | Some m -> check (option (float 1e-9)) "orig timeout" (Some 3.0) (Deadline.original_timeout m)
  | None -> fail "merge_and_timeouts of finite should return"

let to_string () =
  check string "unset string" "Deadline(timeout=none)" (Deadline.to_string (Deadline.create None));

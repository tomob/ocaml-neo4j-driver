...
let point_y p = p.y
let point_z p = p.z
let point_coordinates p = match p.z with None -> [ p.x; p.y ] | Some z -> [ p.x; p.y; z ]
let point_is_cartesian p = p.srid = cartesian_2d_srid || p.srid = cartesian_3d_srid
let point_is_wgs84 p = p.srid = wgs84_2d_srid || p.srid = wgs84_3d_srid

(* --- Vector --- *)

let vector_dtype_size = function I8 -> 1 | I16 -> 2 | I32 -> 4 | I64 -> 8 | F32 -> 4 | F64 -> 8
...
let rec map_to_string entries =
  "{"
  ^ String.concat ", " (List.map (fun (k, v) -> Printf.sprintf "%S: %s" k (to_string v)) entries)
  ^ "}"
...
and to_string = function
  ...
  | Node n ->
      Printf.sprintf "Node<%s>{%s}%s" n.element_id (String.concat "," n.labels)
        (map_to_string n.properties)
  | Relationship r -> Printf.sprintf "Relationship<%s>%s" r.element_id (map_to_string r.properties)
  | Unbound_relationship r ->
      Printf.sprintf "UnboundRelationship<%s>%s" r.element_id (map_to_string r.properties)
  | Path p ->
      Printf.sprintf "Path(%s)" (String.concat ", " (List.map (fun n -> n.element_id) p.nodes))
  | Point p ->

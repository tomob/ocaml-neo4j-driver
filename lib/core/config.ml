...
(* Build a config record once all [checks] pass; the failed check names are
   reported in a single [Configuration_error]. *)
let checked error_prefix checks build =
  match invalid_names checks with
  | [] -> Ok (build ())
  | names -> Error (Errors.Configuration_error (error_prefix ^ String.concat ", " names))
...
let make_workspace_config
  ...
  checked "Invalid workspace config: "
    [
